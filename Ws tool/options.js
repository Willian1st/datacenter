                    ...
                    .bind(
                        ...
                        function (event) {
                            ...
                            if (event.originalEvent.wheelDelta < 0) {
                                target = $(
                                    ".previewHistory li.bg-success ").next();
                            } else {
                                target = $(
                                    ".previewHistory li.bg-success ").prev();
                            }
                            if (target) {
                ...
                $scope.keys = {
                    ...
                    removeObject: function (key, array) {
                        ...
                        if (temp.length < array.length) {
                            ...
                            localStorage.setItem($scope.keys.itemkey,
                                angular
            ...
            function ($scope) {
                $scope.tools = {
                    ...
                    minus: function (data) {
                        ...
                        var nowToShow = nowYear + "-" + nowMonth + "-" + nowDate + " " + nowHour + ":" + nowMinute + ":" + (nowSeconds < 10 ? ("0" + nowSeconds) : nowSeconds);
                        $(".now-to-show").text(nowToShow);
                        var birthSeconds = birth.getSeconds();
                        var birthMinute = birth.getMinutes();
                        var birthHour = birth.getHours();
                        var birthDate = birth.getDate();
                        var birthMonth = birth.getMonth() + 1;
                        var birthYear = birth.getFullYear();
                        var minusYear = (nowMonth - birthMonth) > 0 ? (nowYear - birthYear) : (nowYear - birthYear) - 1;
                        var minusMonth = (nowYear - birthYear) * 12 + (nowMonth - birthMonth);
                        var minusDay = (nowDate - birthDate) > 0 ? (nowDate - birthDate) : (nowDate - birthDate) + new Date(nowYear, nowMonth - 1, 0).getDate();
                        var minusHour = (nowHour - birthHour) > 0 ? (nowHour - birthHour) : 24 + (nowHour - birthHour);
                        var minusMinute = (nowMinute - birthMinute) > 0 ? (nowMinute - birthMinute) : 60 + (nowMinute - birthMinute);
                        var minusSecond = (nowSeconds - birthSeconds) > 0 ? (nowSeconds - birthSeconds) : 60 + (nowSeconds - birthSeconds);
                        if ((nowSeconds - birthSeconds) < 0) {
                            minusMinute -= 1;
                        }
                        if ((nowMinute - birthMinute) < 0) {
                            minusHour -= 1;
                        }
                        if ((nowHour - birthHour) < 0) {
                            minusDay -= 1;
                        }
                        if ((nowDate - birthDate) < 0) {
                            minusMonth -= 1;
                        }
                        $(".minus-year").text(parseInt(minusYear));
                        $(".minus-month").text(parseInt(minusMonth));
                        $(".minus-day").text(parseInt(minusDay));
                        $(".minus-hour").text(parseInt(minusHour));
                        $(".minus-minute").text(parseInt(minusMinute));
                        $(".minus-second").text(minusSecond < 10 ? ("0" + minusSecond) : minusSecond);
                        var birthTime = birth.getTime();
                        var minus = nowTime - birthTime;
                        var day = minus / (1000 * 60 * 60 * 24);
                        var hour = minus / (1000 * 60 * 60);
                        var minute = minus / (1000 * 60);
                        var second = minus / (1000);
                        $(".total-day").text(parseInt(day));
                        $(".total-hour").text(parseInt(hour));
                        $(".total-minute").text(parseInt(minute));
                        $(".total-second").text(parseInt(second));
                    },
                    play: function (data) {
                        var player = document
