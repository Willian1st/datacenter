...
public class UpperCaseActivity extends Activity implements OnClickListener {
	...
	public void onClick(View view) {
		...
		switch (view.getId()) {
		case R.id.button:
			String num=input_value.getText().toString().trim();
			if("".equals(num)){
				Toast.makeText(getApplicationContext(), "请输入数字!", Toast.LENGTH_SHORT).show();
				return;
			}
			BigDecimal numberOfMoney = new BigDecimal(num);
			try {
				result_value.setText(number2CNMontray(numberOfMoney));
			} catch (Exception e) {
				result_value.setText("超出可转换范围...");
			}
			break;
		}
	}
