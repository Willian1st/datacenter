	...
	function style_html(html_source, indent_size, indent_character, max_char) {
		...
		function Parser() {
			...
			this.get_script = function() {
				...
				var end_script = reg_array ? reg_array.index
						: this.input.length;
			...
			this.retrieve_tag = function(tag) {
				if (this.tags[tag + 'count']) {
					...
					if (temp_parent) {
						this.indent_level = this.tags[tag
								+ this.tags[tag + 'count']];
			...
			this.get_tag = function() {
				...
				var tag_check = tag_complete.substring(1, tag_index)
						.toLowerCase();
				if (tag_complete.charAt(tag_complete.length - 2) === '/'
						|| this.Utils.in_array(tag_check,
			...
			this.get_token = function() {
				...
				if (this.last_token === 'TK_TAG_SCRIPT') {
					...
					// token = js_beautify(temp_token, this.indent_size,
					// this.indent_character, this.indent_level);
	...
	return function(data) {
		var dataHolder = [
				...
				[ Math.random(), Math.random(), Math.random(), Math.random() ]
						.join('_').replace(/[^0-9]/g, '_'), '_' ].join('_');
		var dataHolders = {};
		var index = 0;
